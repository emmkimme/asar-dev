...
module.exports.createPackageWithOptions = async function (src, dest, options) {
  ...
  globOptions.follow = options.follow

  const pattern = src + (options.pattern ? options.pattern : '/**/*')

  src = path.normalize(src)
  dest = path.normalize(dest)
  const [filenames, metadata] = await crawlFilesystem(src, pattern, globOptions)
  return _createPackageFromFiles(src, dest, filenames, metadata, options)
}
...
const _createPackageFromFiles = async function (src, dest, filenames, metadata, options) {
  ...
  const handleFile = async function (filename) {
    if (!metadata[filename]) {
      metadata[filename] = await crawlFilesystem.determineFileType(src, filename, options)
    }
